...
# SET obs4MIPs records to test te check_replica script
update_dict = {'project:obs4MIPs':
               {'version': '1',
                'latest': ['true'],
                'replica': ['false']}}
update_solr(update_dict, update='set', solr_url=solr_url, solr_core='datasets')

# SET a field values (i.e. will override values for fields with the same name,
# or insert new ones if not existing already)
update_dict = {'project:obs4MIPs': {'location': ['Pasadena'],
                                    'realm': ['atmosphere']}}
update_solr(update_dict, update='set', solr_url=solr_url, solr_core='datasets')

# ADD values to existing fields, or add new fields with values
update_dict = {'project:obs4MIPs&source_id:MLS': {'location': ['Boulder'],
                                                  'stratus': ['cumulus']}}
...
# transfer the value of a field to a new named field
update_dict = {'project:obs4MIPs': {'activity_id': ['$project']}}
update_solr(update_dict, update='set', solr_url=solr_url, solr_core='datasets')

# change version, replica, latest
update_dict = {'project:obs4MIPs': {'replica': ['true'],
